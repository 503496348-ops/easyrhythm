"""Tests for easyrhythm CLI entry point."""
...
def test_help():
    ...
    assert 'easyrhythm' in r.stdout.lower() or 'usage' in r.stdout.lower() or len(r.stdout) > 20
...
def test_classify():
    """classify command should accept text."""
    r = subprocess.run([sys.executable, os.path.join(SCRIPTS, 'cli.py'), 'classify', 'hello'],
                       capture_output=True, text=True, timeout=10)
    assert r.returncode == 0
